        ...
        try:
            ...
            if resuming_approval:
                ...
                try:
                    ...
                    agent_logger.log_info(
                        "Workflow Execution",
                        f"Current state interrupts: {len(current_state.interrupts) if hasattr(current_state, 'interrupts') else 'No interrupts'}",
                    )
                    agent_logger.log_info(
                        "Workflow Execution",
                        f"Current state next: {current_state.next if hasattr(current_state, 'next') else 'No next'}",
                    )
                    agent_logger.log_info(
                        "Workflow Execution",
                        f"Current state values keys: {list(current_state.values.keys()) if hasattr(current_state, 'values') and current_state.values else 'No values'}",
                    )
                except Exception as e:
                    agent_logger.log_info(
                        "Workflow Execution", f"Could not get state: {e}"
                    )
