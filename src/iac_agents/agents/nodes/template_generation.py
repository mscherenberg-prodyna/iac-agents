...
def template_generation_node(state: InfrastructureStateDict) -> InfrastructureStateDict:
    ...
    try:
        ...
        template_response = f"Generated template for: {user_input}\n\n```hcl\n{template}\n```"
        
        if not template:
            log_warning("Template Generation Node", "No template extracted, using fallback")
            template = template_manager.get_fallback_template(user_input)
        ...
        result = TemplateGenerationResult(
            ...
            data={
                ...
                "extraction_method": "hcl_blocks" if "```hcl" in template_response else "fallback"
            },
