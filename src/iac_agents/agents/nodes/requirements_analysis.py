...
def requirements_analysis_node(
    ...
    log_agent_complete(
        "Requirements Analysis Node", "Requirements analyzed", extracted_info
    )
...
def _estimate_complexity(user_input: str) -> int:
    ...
    scores = [
        score for keyword, score in complexity_keywords.items() if keyword in user_lower
    ]
...
def _determine_workflow_stages(analysis: Dict[str, Any]) -> List[str]:
    ...
    stages.extend(
        [WorkflowStage.APPROVAL_PREPARATION.value, WorkflowStage.COMPLETED.value]
    )
