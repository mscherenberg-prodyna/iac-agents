...
def approval_preparation_node(state: InfrastructureStateDict) -> InfrastructureStateDict:
    ...
    try:
        ...
        approval_summary = {
            "total_cost": cost_estimation.get("data", {}).get("cost_estimate", {}).get("total_monthly_usd", 0),
            "compliance_score": state.get("compliance_score", 0),
            "resource_count": template.count("resource ") if template else 0,
            "frameworks_validated": compliance_validation.get("data", {}).get("validation_frameworks", []),
        ...
        log_warning("Approval Preparation Node", f"Approval preparation failed: {str(e)}")
