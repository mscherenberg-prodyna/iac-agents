...
def validation_compliance_node(state: InfrastructureStateDict) -> InfrastructureStateDict:
    ...
    if not template:
        log_warning("Validation Compliance Node", "No template available for validation")
        return {
            **state,
            "current_stage": WorkflowStage.VALIDATION_AND_COMPLIANCE.value,
            "errors": state.get("errors", []) + ["No template available for validation"],
        }
    ...
    if compliance_settings.get("enforce_compliance"):
        ...
        validation_frameworks = requirements_analysis.get("compliance_requirements", ["GDPR"])
    
    # Run compliance validation
    compliance_result = compliance_framework.validate_template(template, validation_frameworks)
    ...
    quality_gate_passed = (
        compliance_score >= minimum_score_threshold and
        violations_count <= maximum_violations_threshold
    ) if should_enforce_quality_gates else True
    ...
    validation_result = ComplianceValidationResult(
        ...
        passed_checks=basic_validation.get("passed_checks", []) if isinstance(basic_validation, dict) else []
    )
