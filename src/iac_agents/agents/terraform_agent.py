...
class TerraformAgent:
    ...
    def _initialize_llm(self):
        ...
        if azure_endpoint and azure_key and azure_deployment:
            ...
            return ChatOpenAI(
                model="gpt-4", temperature=config.agents.terraform_agent_temperature
            )
    ...
    def _generate_terraform_template(
        self, requirements: str, research_context: str = ""
    ) -> str:
    ...
    def _validate_template(self, template: str) -> Dict[str, Any]:
        ...
        if "password" in template.lower() and "var." not in template.lower():
            validation_result["security_notes"].append(
                "Consider using variables for sensitive data"
            )

        if "admin" in template.lower():
            validation_result["security_notes"].append(
                "Review admin access configurations"
            )
