...
class TerraformResearchAgent:
    ...
    def _generate_search_queries(self, user_query: str) -> List[str]:
        ...
        for provider in cloud_providers:
            if provider in user_query.lower():
                query = (f"terraform {provider} {user_query} "
    ...
    def _synthesize_research(self, original_query: str,
...
Provide concise guidance for implementing this Terraform configuration.""")
    ...
    def _generate_fallback_guidance(self, query: str) -> str:
        ...
        if any(keyword in query_lower for keyword in ["document", "file", "storage", "legal", "compliance"]):
            return """
    ...
    def get_provider_best_practices(self,
                                    provider: str = "azurerm") -> Dict[str, List[str]]:
    ...
    def validate_terraform_syntax(self, template: str) -> Dict[str, any]:
        ...
        if ("password" in template.lower() and
                "random_password" not in template.lower()):
    ...
    def _extract_azure_resources(self, template: str) -> Dict[str, List[Dict[str, str]]]:
    ...
    def _estimate_resource_cost(self, resource_type: str, instances: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        cost_data = {
            "azurerm_app_service_plan": {
                "basic": {"monthly_usd": 13.87, "description": "Basic B1 (1 Core, 1.75 GB RAM)"},
                "standard": {"monthly_usd": 55.48, "description": "Standard S1 (1 Core, 1.75 GB RAM)"},
                "premium": {"monthly_usd": 166.44, "description": "Premium P1v2 (1 Core, 3.5 GB RAM)"}
            },
            "azurerm_app_service": {
                "basic": {"monthly_usd": 0, "description": "Included with App Service Plan"},
            },
            "azurerm_sql_database": {
                "basic": {"monthly_usd": 5.88, "description": "Basic DTU"},
                "standard": {"monthly_usd": 29.40, "description": "Standard S1 (20 DTUs)"},
                "premium": {"monthly_usd": 441, "description": "Premium P1 (125 DTUs)"}
            },
            "azurerm_sql_server": {
                "basic": {"monthly_usd": 0, "description": "No additional cost for logical server"}
            },
            "azurerm_storage_account": {
                "basic": {"monthly_usd": 20.48, "description": "Standard LRS (100 GB)"},
                "premium": {"monthly_usd": 102.40, "description": "Premium LRS (100 GB)"}
            },
            "azurerm_virtual_machine": {
                "small": {"monthly_usd": 60.74, "description": "Standard B2s (2 vCPUs, 4 GB RAM)"},
                "medium": {"monthly_usd": 175.20, "description": "Standard D4s v3 (4 vCPUs, 16 GB RAM)"},
                "large": {"monthly_usd": 350.40, "description": "Standard D8s v3 (8 vCPUs, 32 GB RAM)"}
            },
            "azurerm_kubernetes_cluster": {
                "basic": {"monthly_usd": 73.00, "description": "Managed Kubernetes + 2 Standard B2s nodes"},
                "standard": {"monthly_usd": 219.00, "description": "Managed Kubernetes + 3 Standard D4s v3 nodes"}
            },
            "azurerm_cosmosdb_account": {
                "basic": {"monthly_usd": 24.63, "description": "400 RU/s provisioned throughput"},
                "standard": {"monthly_usd": 58.71, "description": "1000 RU/s provisioned throughput"}
            },
            "azurerm_redis_cache": {
        ...
        tier = "standard" if "standard" in cost_data[resource_type] else list(cost_data[resource_type].keys())[0]
    ...
    def _generate_cost_summary(self, total_monthly: float, breakdown: Dict[str, Any]) -> str:
        ...
        if breakdown:
            ...
            for resource_type, cost_data in breakdown.items():
                resource_name = resource_type.replace("azurerm_", "").replace("_", " ").title()
                monthly_cost = cost_data["monthly_usd"]
                instances = cost_data["instances"]
                
                if monthly_cost > 0:
                    instance_text = f" x{instances}" if instances > 1 else ""
                    summary += f"- {resource_name}{instance_text}: ${monthly_cost:.2f}/month\n"
        
        summary += f"\n💡 **Note:** Costs may vary based on actual usage, region, and specific configurations."
        return summary
    
    def _assess_cost_confidence(self, resources: Dict[str, List[Dict[str, str]]]) -> str:
