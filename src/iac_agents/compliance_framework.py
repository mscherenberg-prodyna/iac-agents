...
class ComplianceFramework:
    ...
    def validate_template(self, template: str, required_frameworks: List[str] = None) -> Dict[str, Any]:
        ...
        compliance_score = (passed_count / total_rules * 100) if total_rules > 0 else 100
        ...
        return {
            ...
            "framework_compliance": self._calculate_framework_compliance(violations, required_frameworks or []),
    ...
    def _calculate_framework_compliance(self, violations: List[ComplianceViolation], 
    ...
    def _prioritize_remediation(self, violations: List[ComplianceViolation]) -> List[ComplianceViolation]:
...
"""
        ...
        if framework_scores:
            ...
            for framework, framework_score in framework_scores.items():
                status_emoji = "✅" if framework_score >= 90 else "⚠️" if framework_score >= 70 else "❌"
        ...
        if critical_violations:
            ...
            for violation in critical_violations:
                report += f"- **{violation.rule_name}** ({', '.join(violation.frameworks)})\n"
        ...
        other_violations = [v for v in violations if v.level in [ComplianceLevel.MEDIUM, ComplianceLevel.LOW]]
        if other_violations:
