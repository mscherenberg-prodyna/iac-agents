...
class TerraformTemplateValidator:
    ...
    def __init__(self):
        ...
        self.azure_best_practices = ["resource_group", "tags", "location"]
            ...
            for pattern in patterns:
                ...
                if matches:
                    security_issues.append(
                        f"Potential {issue_type.replace('_', ' ')}: {len(matches)} occurrence(s)"
                    )

        # Check for missing encryption
        if (
            "storage_account" in template.lower()
            and "enable_https_traffic_only" not in template.lower()
        ):
            security_issues.append("Storage account should enforce HTTPS traffic")

        if (
            "virtual_machine" in template.lower()
            and "disable_password_authentication" not in template.lower()
        ):
    ...
    def _check_best_practices(self, template: str) -> List[str]:
        ...
        if (
            re.search(r'provider\s+"[^"]*"\s*{', template)
            and "version" not in template.lower()
        ):
        ...
        if "azurerm" in template.lower():
            ...
            if len(resource_blocks) > len(tagged_resources):
                suggestions.append(
                    "Consider adding tags to Azure resources for better management"
                )

        # Check for naming conventions
        if re.search(r'name\s*=\s*"[A-Z]', template):
            suggestions.append(
                "Consider using lowercase names following cloud provider conventions"
            )
        ...
        if template.count("resource") > 3 and "depends_on" not in template:
            suggestions.append(
                "Consider explicit dependencies for complex resource relationships"
            )
    ...
    def validate_for_deployment(self, template: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        deployment_ready = (
            ...
            and self._has_required_deployment_elements(template)
        )
