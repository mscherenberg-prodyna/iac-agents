...
def test_workflow_progress_display_formatting(mock_st):
    ...
    stage_calls = [call for call in calls if "Template Generation" in str(call) or "Requirements Analysis" in str(call)]
...
def test_workflow_progress_error_handling():
    ...
    with patch('src.iac_agents.streamlit.components.sidebar.st') as mock_st:
        ...
        try:
            ...
            pytest.fail(f"display_workflow_progress should handle errors gracefully, but raised: {e}")
