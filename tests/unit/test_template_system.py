...
def test_template_loader_lists_files():
    ...
    for template in expected_tf_templates:
        assert template in terraform_templates, f"Missing terraform template: {template}"
...
def test_template_loader_loads_files():
    ...
    for template_name in ["document_storage", "web_application", "default"]:
        ...
        assert "terraform" in template_content, f"Template {template_name} should contain terraform block"
        assert "provider" in template_content, f"Template {template_name} should contain provider block"
    
    # Test loading prompt templates
    for prompt_name in ["terraform_system", "enhanced_terraform", "requirements_analysis"]:
...
def test_template_manager_get_prompt():
    ...
    formatted_prompt = tm.get_prompt("enhanced_terraform", 
...
def test_template_manager_get_terraform_template():
    ...
    for template_name in ["document_storage", "web_application", "default"]:
        ...
        assert "terraform" in template, f"Template {template_name} should contain terraform block"
