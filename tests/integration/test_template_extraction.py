...
def test_template_extraction_through_workflow():
    ...
    if end > start:
        ...
        assert "terraform" in template.lower() or "provider" in template.lower() or "resource" in template.lower(), \
               "Template should contain Terraform configuration elements"
        assert "azurerm" in template.lower() or "azure" in template.lower(), \
...
def test_workflow_template_generation_stage():
    ...
    for test_case in test_cases:
        ...
        infrastructure_keywords = ["terraform", "resource", "provider", "azure", "infrastructure"]
        assert any(keyword in response_lower for keyword in infrastructure_keywords), \
        ...
        assert "template_generation" in completed_stages, \
...
def test_template_validation_integration():
    ...
    status = supervisor.get_workflow_status()
    completed_stages = status.get("completed_stages", [])
    assert "validation_and_compliance" in completed_stages, \
...
def test_multiple_template_extraction_patterns():
    ...
    for request in test_requests:
        ...
        assert has_template or has_explanation, \
